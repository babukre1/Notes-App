    ...
    if (!hash_equals($_SESSION['csrf'], $csrf)) {
        ...
        try {
            ...
            if (!$note) {
                ...
                $_SESSION['csrf'] = bin2hex(random_bytes(32));
                header("Location: index.php?id={$noteId}");
                exit;
            }
        } catch (Throwable $t) {
            $errors[] = 'Something went wrong. Please try again.';
...
<body>
    ...
    <div class="wrapper">
        <nav class="nav-sidebar" style="display: flex; flex-direction: column; height: 100%; padding: 20px 0;">
            <a href="index.php?view=settings" style="margin-bottom: 20px; text-align: center; text-decoration: none;">
            ...
            <a href="index.php?new=true" class="new-btn">+</a>

            <a href="index.php" class="nav-icon <?= $filter === 'all' && $view !== 'settings' ? 'active' : '' ?>"
                title="All Notes">📓</a>
            <a href="index.php?filter=starred" class="nav-icon <?= $filter === 'starred' ? 'active' : '' ?>"
                title="Starred">⭐</a>
            <a href="index.php?view=settings" class="nav-icon <?= $view === 'settings' ? 'active' : '' ?>"
                title="Settings">⚙️</a>
        ...
        <div class="editor-pane">
            ...
            <?php if ($view === 'settings'): ?>
                <div class="settings-container">
                    ...
                    <form action="index.php?view=settings" method="POST" enctype="multipart/form-data">
